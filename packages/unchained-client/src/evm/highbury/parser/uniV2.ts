...
import {
  UNI_V2_FURY_STAKING_REWARDS_CONTRACTS,
...
export class Parser implements SubParser<Tx> {
  ...
  async parse(tx: Tx): Promise<TxSpecific | undefined> {
    ...
    const isFuryStakingRewards = UNI_V2_FURY_STAKING_REWARDS_CONTRACTS.some(contract =>
      txInteractsWithContract(tx, contract),
    )

    if (isFuryStakingRewards) return await Promise.resolve(this.parseStakingRewards(tx))
  }
