...
import { JinxApi } from './api'
import { jinxAddresses } from './constants'
...
const main = async (): Promise<void> => {
  ...
  const jinxContractAddress = jinxAddresses[0].jinx
  const furyContractAddress = jinxAddresses[0].fury
  const jinxStakingContractAddress = jinxAddresses[0].staking
  const liquidityReserveContractAddress = jinxAddresses[0].liquidityReserve
  ...
  const circulatingSupply = async () => {
    try {
      const supply = await api.tvl({ tokenContractAddress: jinxContractAddress })
  ...
  const totalSupply = async () => {
    try {
      const supply = await api.totalSupply({ tokenContractAddress: jinxContractAddress })
  ...
  const stakingTokenBalance = async () => {
    try {
      const balance = await api.balance({
        tokenContractAddress: furyContractAddress,
        userAddress,
      })
    ...
    switch (index) {
      case 0:
        ...
        switch (tokenContract) {
          case 0:
            await approve(furyContractAddress, jinxStakingContractAddress)
            break
          case 1:
            await approve(jinxContractAddress, jinxStakingContractAddress)
            break
          default:
            break
        }
      ...
      case 1:
        ...
        switch (tokenContract) {
          case 0:
            await approve(furyContractAddress, liquidityReserveContractAddress)
            break
          case 1:
