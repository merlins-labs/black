...
import { jinxAbi } from '../abi/jinx-abi'
import { jinxStakingAbi } from '../abi/jinx-staking-abi'
...
export type ConstructorArgs = {
  ...
  providerUrl: string
  jinxAddresses: JinxAddressesType
  chainReference?: EthereumChainReference
}

export const transformData = ({ tvl, apy, expired, ...contractData }: JinxOpportunityInputData) => {
  return {
    type: DefiType.Staking,
    provider: 'ShapeShift',
    version: '1',
    contractAddress: contractData.staking,
    rewardToken: contractData.jinx,
    stakingToken: contractData.fury,
...
export class JinxApi {
  ...
  constructor({
    ...
    this.jinxStakingContracts = jinxAddresses.map(
      addresses => new ethers.Contract(addresses.staking, jinxStakingAbi, this.provider),
    )
    this.liquidityReserveContracts = jinxAddresses.map(
      addresses =>
    ...
    this.jinxAddresses = jinxAddresses
  }
  ...
  private getStakingContract(contractAddress: string): ethers.Contract {
    const stakingContract = this.jinxStakingContracts.find(
  ...
  async getJinxOpportunities() {
    try {
      const opportunities = await Promise.all(
        this.jinxAddresses.map(async addresses => {
          const stakingContract = this.jinxStakingContracts.find(
            item => toLower(item.address) === toLower(addresses.staking),
          )
          try {
            const expired = await stakingContract?.pauseStaking()
            const tvl = await this.tvl({ tokenContractAddress: addresses.jinx })
            const apy = this.apy()
      ...
      throw new Error(`getJinxOpportunities Error: ${e}`)
    }
  }

  async getJinxOpportunityByStakingAddress(stakingAddress: string) {
    this.verifyAddresses([stakingAddress])
    const addresses = this.jinxAddresses.find(item => {
  ...
  async tvl(input: TokenAddressInput): Promise<BigNumber> {
    ...
    this.verifyAddresses([tokenContractAddress])
    const contract = new ethers.Contract(tokenContractAddress, jinxAbi, this.provider)
  ...
  async getRebaseHistory(input: BalanceInput) {
    ...
    const jinxContract = new ethers.Contract(tokenContractAddress, jinxAbi, this.provider)
    const fromBlock = 14381454 // genesis rebase

    const rebaseEvents = await (async () => {
      try {
        const filter = jinxContract.filters.LogRebase()
        const events = await jinxContract.queryFilter(filter, fromBlock, 'latest')
    ...
    const transferEvents = await (async () => {
      try {
        const filter = jinxContract.filters.Transfer()
        const events = await jinxContract.queryFilter(filter, fromBlock, 'latest')
        return events
      } catch (e) {
      ...
      events.map(async event => {
        const { preRebaseBalance, postRebaseBalance } = await (async () => {
          try {
            ...
            const postRebaseBalanceResult = await jinxContract.balanceOf(userAddress, {
              blockTag: event.blockNumber,
            })
            const unadjustedPreRebaseBalance = await jinxContract.balanceOf(userAddress, {
    ...
    const actualResults = results.reduce<RebaseHistory[]>((acc, cur) => {
      if (cur.status === 'rejected') {
        console.error('getJinxRebaseHistory: balanceOf call failed - charts will be wrong')
        return acc
      }
