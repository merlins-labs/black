...
export const JinxDeposit: React.FC<{
  ...
  const {
    chainId,
    contractAddress: jinxContractAddress,
    assetReference: jinxStakingContractAddress,
    assetNamespace,
  } = query
  // ContractAssetId
  const assetId = toAssetId({ chainId, assetNamespace, assetReference: jinxStakingContractAddress })
    ...
    ;(async () => {
      try {
        ...
        dispatch({
          ...
          payload: { ...jinxOpportunity, apy: jinxAprData?.jinxApr ?? '' },
        })
      } catch (error) {
  ...
  }, [accountId, handleAccountIdChange, jinxContractAddress, translate, stakingAsset.symbol])
