...
export const Deposit: React.FC<DepositProps> = ({
  ...
  const opportunity = useMemo(() => state?.jinxOpportunity, [state])
  ...
  const handleContinue = useCallback(
    async (formValues: DepositValues) => {
      if (!(state && accountAddress && dispatch && jinxApi)) return

      const getApproveGasEstimate = async () => {
        if (!accountAddress || !assetReference || !jinxApi) return
        try {
      ...
      const getDepositGasEstimateCryptoBaseUnit = async (deposit: DepositValues) => {
        if (!accountAddress || !assetReference || !jinxApi) return
        try {
          const feeDataEstimate = await jinxApi.estimateDepositFees({
            tokenContractAddress: assetReference,
            contractAddress,
      ...
      dispatch({ type: JinxDepositActionType.SET_DEPOSIT, payload: formValues })
