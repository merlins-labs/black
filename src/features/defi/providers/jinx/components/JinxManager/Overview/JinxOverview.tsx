...
import { JinxEmpty } from './JinxEmpty'
import { WithdrawCard } from './WithdrawCard'

type JinxOverviewProps = {
  accountId: AccountId | undefined
  onAccountIdChange: AccountDropdownProps['onChange']
}

export const JinxOverview: React.FC<JinxOverviewProps> = ({
  accountId,
  ...
  useEffect(() => {
    if (!maybeAccountId) return
    ;(async () => {
      const canClaimWithdraw = await jinxApi.canClaimWithdraw({
        contractAddress,
        userAddress: fromAccountId(maybeAccountId).account,
      })
      setCanClaim(canClaimWithdraw)
    })()
  }, [contractAddress, jinxApi, maybeAccountId])
  ...
  const cryptoAmountAvailablePrecision = bnOrZero(
    jinxEarnOpportunityData?.stakedAmountCryptoBaseUnit,
  ...
  if (
    bnOrZero(jinxEarnOpportunityData?.stakedAmountCryptoBaseUnit).eq(0) &&
    !canClaim &&
    ...
    return (
      <JinxEmpty
        ...
        apy={jinxEarnOpportunityData?.apy ?? ''}
        onClick={() =>
          history.push({
  ...
  return (
    <Overview
      ...
      provider={makeDefiProviderDisplayName({
        provider: jinxEarnOpportunityData.provider,
      ...
      tvl={bnOrZero(jinxEarnOpportunityData?.tvl).toFixed(2)}
      apy={jinxEarnOpportunityData?.apy?.toString()}
    >
      <WithdrawCard
        asset={stakingAsset}
