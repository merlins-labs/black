...
export type JinxWithdrawValues = {
...
> = ({ accountId, onAccountIdChange: handleAccountIdChange, onNext }) => {
  ...
  const methods = useForm<JinxWithdrawValues>({ mode: 'onChange' })
  ...
  const handleContinue = useCallback(
    async (formValues: JinxWithdrawValues) => {
      if (!(accountAddress && dispatch && rewardId && jinxApi && bip44Params)) return

      const getApproveGasEstimateCryptoBaseUnit = async () => {
        if (!accountAddress) return

        try {
          const feeDataEstimate = await jinxApi.estimateApproveFees({
      ...
      const getWithdrawGasEstimateCryptoBaseUnit = async (withdraw: JinxWithdrawValues) => {
        if (!accountAddress) return

        try {
          const feeDataEstimate = await jinxApi.estimateWithdrawFees({
            tokenContractAddress: rewardId,
            contractAddress,
      ...
      try {
        ...
        const _allowance = await jinxApi.allowance({
          tokenContractAddress: rewardId,
          contractAddress,
          userAddress: accountAddress,
        })
    ...
    <FormProvider {...methods}>
      ...
      >
        <WithdrawTypeField
          asset={stakingAsset}
          handlePercentClick={handlePercentClick}
          feePercentage={bnOrZero(state.jinxFeePercentage).toString()}
        />
      </ReusableWithdraw>
    </FormProvider>
  )
