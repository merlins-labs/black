...
export const Deposit: React.FC<DepositProps> = ({
  ...
  const underlyingAssetId = furyFarmingOpportunity?.underlyingAssetId
  ...
  const { getLpTokenPrice } = useUniV2LiquidityPool({
    accountId: accountId ?? '',
    assetId0: furyFarmingOpportunity?.underlyingAssetIds[0] ?? '',
    assetId1: furyFarmingOpportunity?.underlyingAssetIds[1] ?? '',
  ...
  const {
    allowance: furyFarmingAllowance,
    getStakeFees,
    getApproveFees,
  } = useFuryFarming(contractAddress)
  ...
  const handleContinue = useCallback(
    async (formValues: DepositValues) => {
      if (!(state && dispatch && feeAsset && furyFarmingOpportunity)) return
      ...
      dispatch({ type: FuryFarmingDepositActionType.SET_DEPOSIT, payload: formValues })
      dispatch({ type: FuryFarmingDepositActionType.SET_LOADING, payload: true })
      try {
        if (!asset) return
        // Check if approval is required for user address
        const _allowance = await furyFarmingAllowance()
  ...
  if (!state || !dispatch || !furyFarmingOpportunity || !asset || !marketData) return null
  ...
  return (
    <ReusableDeposit
      ...
      inputIcons={furyFarmingOpportunity?.icons}
      apy={String(furyFarmingOpportunity?.apy)}
