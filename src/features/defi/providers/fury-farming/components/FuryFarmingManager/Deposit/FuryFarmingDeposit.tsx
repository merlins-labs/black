...
type FuryFarmingDepositProps = {
  onAccountIdChange: AccountDropdownProps['onChange']
  accountId: AccountId | undefined
}
export const FuryFarmingDeposit: React.FC<FuryFarmingDepositProps> = ({
  accountId,
  ...
  useEffect(() => {
    ;(() => {
      try {
        if (!(farmingAccountId && contractAddress && furyFarmingOpportunity)) return
      } catch (error) {
        // TODO: handle client side errors
        console.error(error)
      }
    })()
  }, [farmingAccountId, translate, toast, contractAddress, furyFarmingOpportunity])
  ...
  if (loading || !marketData || !furyFarmingOpportunity || !StepConfig) {
    return (
      ...
      <DefiModalContent>
        <DefiModalHeader
          title={translate('modals.deposit.depositInto', {
            opportunity: furyFarmingOpportunity.name,
          })}
          onBack={handleBack}
        />
