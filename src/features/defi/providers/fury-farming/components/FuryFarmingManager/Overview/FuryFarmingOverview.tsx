...
import { FuryFarmingEmpty } from './FuryFarmingEmpty'
import { WithdrawCard } from './WithdrawCard'

type FuryFarmingOverviewProps = {
  accountId?: AccountId | undefined
  onAccountIdChange: AccountDropdownProps['onChange']
}

export const FuryFarmingOverview: React.FC<FuryFarmingOverviewProps> = ({
  accountId,
  ...
  const lpAsset = assets[furyEthLpAssetId]
  ...
  const underlyingAssetsFiatBalance = useMemo(() => {
    ...
    const furyEthLpFiatPrice = marketData?.[opportunityData?.underlyingAssetId ?? '']?.price ?? '0'
    return bnOrZero(cryptoAmount).times(furyEthLpFiatPrice).toString()
  }, [
    marketData,
